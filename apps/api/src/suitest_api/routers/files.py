...
_FILE_WRITER_ROLES: frozenset[Role] = frozenset({Role.QA, Role.ADMIN, Role.OWNER})


def _require_file_writer(
    ctx: TenantContext = Depends(tenant_via_api_key_or_session),
) -> TenantContext:
    if ctx.role not in _FILE_WRITER_ROLES:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Permission denied: QA role or higher is required to upload or delete workspace files.",
        )
    return ctx
