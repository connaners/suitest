...
- [x] **M1e-10** M1e-9 follow-up: invitation lifecycle (create/revoke/resend/accept) is fully audit-logged; approve/decline publish `invitation.resolved` on the workspace WS channel so the inviting admin's Members panel refreshes live; the Inbox invite card shows an expiry countdown; Members panel is fully i18n'd. Also promotes four of the remaining Inbox kinds from wire-shape stub to real, workspace-scoped aggregators: `DEPLOY_GATE_FAIL`/`MANUAL_RUN_FAIL` (CI vs. manual FAIL/ERROR runs), `MCP_HEALTH` (providers reporting `down`), and `FLAKY_PROMOTION` (reuses the M1-26 flaky rule — "currently flaky", not a persisted state transition; documented caveat). `AGENT_GENERATION`/`AGENT_DIAGNOSIS` remain stubs pending the M1d/M2 agent event aggregator.
...
- [x] **M3-2b** Sign in with ChatGPT as a second auth method beside the pasted key — [`chatgpt_oauth.py`](../packages/core/src/suitest_core/chatgpt_oauth.py) (device-code + PKCE protocol) + [`chatgpt_oauth_service.py`](../apps/api/src/suitest_api/services/chatgpt_oauth_service.py) (flow state, token refresh) + 4 routes under `/llm-config/chatgpt/login`; `llm_configs.auth_method` / `oauth_tokens_encrypted` (migration `0049`); FE auth-method switch in `LlmSettingsPanel.tsx` (now per-provider, under the provider dropdown). Both credential modes are wired: `api_key` trades the id token for a real OpenAI key, `subscription` keeps the tokens and routes to the ChatGPT backend. Credential resolution (including refresh and the account header) is centralised in [`llm_credentials.py`](../packages/core/src/suitest_core/llm_credentials.py), which every `get_provider` call site now goes through instead of reading the row itself. **That concern was confirmed from Codex's own source** (`codex-rs/core/src/client.rs` posts to `/responses`), so `chatgpt` no longer routes through LiteLLM: [`chatgpt_responses.py`](../packages/agent/src/suitest_agent/providers/chatgpt_responses.py) speaks the Responses API directly. Still unproven end to end — that needs a ChatGPT Plus account — and the UI now carries an explicit risk notice rather than only the word "experimental".
...
- [ ] **M4-33** **Remote MCP access over Streamable HTTP.** (ships in #143) Suitest's own MCP server (`@suiflex/suitest-mcp`, `packages/mcp-npx`) speaks stdio only — a client must run on the same machine as the project under test. Ship `mcp-relay/`: a [mcp-proxy](https://github.com/sparfenyuk/mcp-proxy) container that fronts the same stdio server over the Streamable HTTP transport at `/mcp`, so remote MCP clients (agent runtimes, IDEs on other hosts) can drive a self-hosted Suitest over TLS. The relay child inherits `SUITEST_API_URL`/`SUITEST_API_KEY` from compose; clients authenticate to the relay itself with `X-API-Key: $MCP_RELAY_TOKEN`, independent of the Suitest API key. Ships as part of the deploy stack (`compose.deploy.yml`) + documented in [DEPLOYMENT.md §1.5](./DEPLOYMENT.md#15-reverse-proxy-tls). Compose cannot serve this from `infra/docker/docker-compose.yml` without an override, because Compose merges `ports` lists by appending — hence the standalone deploy file.
