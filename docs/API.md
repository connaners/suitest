...
| GET/PATCH | `/users/me` | FastAPI-Users' own route (unprefixed, NOT under `/api/v1`). `PATCH` accepts `{name}` — the Suitest-added field on `UserRead`/`UserUpdate` — to rename the signed-in user. `email`/`password` fields exist on the schema but are unused by the web UI, which changes those via the dedicated routes above |
...
`{"variant":"antigravity"}`; a client is bundled for it, and
`SUITEST_LLM_ANTIGRAVITY_OAUTH_CLIENT_ID` / `_SECRET` only override that). `vertex`
...
| POST | `/agent/chat` | **(M3-12/M3-13)** Conversation mode — streams `token` SSE frames; tool requests emit a `tool` frame + `agent.tool.call` WS event. Optional `model` asks this turn of one model the provider offers (the workspace config is not changed); **400** when it is not one. **409** when no active LLM | **Yes (CLOUD/LOCAL)** |
