...
def gh_author(sha):
    ...
    One gh api call per bullet, cached by sha. A release is a handful of
...
def attribute(text, resolve=gh_author, history=prev_emails, product="Suitest"):
    ...
    if prevtag and newcomers:
        block += "\n\n### New Contributors\n\n"
        block += "\n".join(f"* {t} made their first contribution" for t in newcomers) + "\n"
...
def main():
    ...
    product = os.environ.get("PRODUCT_NAME", "Suitest")
